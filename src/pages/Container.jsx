import React, { useState, useEffect } from 'react';
import "./css/container.css";
import goods from "../good";
import { useNavigate } from 'react-router-dom';
import Footer from './Footer';
import Slide from '@mui/material/Slide';
import { Button } from '@mui/material';

export default function Container() {
  const navigate = useNavigate();
  const [isVisible, setIsVisible] = useState(false);

  // 提取所有不重复的商品类型（从goods中自动获取）
  const productTypes = Array.from(new Set(goods.map(item => item.type)));
  // 类型名称映射（对应显示标题，与Catalog组件一致）
  const typeNameMap = {
    stuffed: "Stuffed Animals",
    wooden: "Wooden Toys"
  };

  const tz = (id) => {
    navigate(`/product/${id}`);
  };

  const handClick = () => {
    navigate('/catalog');
  };

  // 组件挂载后触发动画
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
    setIsVisible(true);
  }, []);

  return (
    <div>
      <div className='main'>
        <div className='content'>
          <Slide
            direction="up"
            in={isVisible}
            timeout={800}
            mountOnEnter
            unmountOnExit
          >
            <div className="banner">
              <div className="banner-container">
                <p>Say Hello to ToyStore!</p>
                <h1>Welcome to ToyStore</h1>
                <Button
                  sx={{
                    height: "48px",
                    padding: "12px 28px",
                    borderRadius: "24px",
                    backgroundColor: "#a5c926",
                    color: "#fff",
                    fontSize: "14px",
                    lineHeight: "24px",
                    fontWeight: 600,
                  }}
                  variant="contained"
                  onClick={handClick}
                >
                  Open Catalog
                </Button>
              </div>
            </div>
          </Slide>

          <div className="nricon">
            <div className="nrcontain">
              <div className="cril"></div>
            </div>
          </div>

        </div>
        <div className="cont-main">
          <div className="con-card">
            <div className="card1">
              <div className="cardImg">
                <img src="https://assets.website-files.com/5badda2935e11303a89a461e/5badf21356ac5470c84dfbf4_33903-2-plush-toy-transparent-image-min-p-500.png" alt="" />
              </div>
              <div className="cardText">
                <h3>Stuffed Animals</h3>
                <a href="/catalog">Shop Now</a>
              </div>
            </div>
            <div className="card2">
              <div className="cardText">
                <h3>Wooden Toys</h3>
                <a href="/catalog">Shop Now</a>
              </div>
              <div className="cardImg">
                <img src="https://assets.website-files.com/5badda2935e11303a89a461e/5badf2131f2da24c02171c72_33727-9-wooden-toy-transparent-image-min-p-500.png" alt="" />
              </div>
            </div>
          </div>

          {/* 动态遍历所有商品类型，每个类型显示前4个商品 */}
          <div className="container main-con">
            {productTypes.map((type) => (
              <div key={type} className="suff">
                <div className="cont-main-top">
                  <div className="words">
                    {/* 动态显示类型名称 */}
                    <h3>{typeNameMap[type] || type}</h3>
                    <a href="/catalog">See All Toys</a>
                  </div>
                  <div className="line">
                    <div className="co"></div>
                  </div>
                </div>
                <div className="cont-main-main">
                  <div className="lists">
                    <ul>
                      {/* 筛选当前类型的商品，截取前4个 */}
                      {goods
                        .filter(item => item.type === type)
                        .slice(0, 4) // 限制只显示4个
                        .map((item) => (
                          <li key={item.id} className='list' onClick={() => tz(item.id)}>
                            <img src={item.img} alt={item.title} />
                            <div className="text">
                              <h3>{item.title}</h3>
                              <div className="p">
                                <p>$&nbsp;{item.price.toFixed(2)}&nbsp;USD</p>
                              </div>
                            </div>
                          </li>
                        ))}
                    </ul>
                  </div>
                </div>
              </div>
            ))}
          </div>

          <div className="xsphoto">
            <div className="xsmain">
              <p>About The Shop</p>
              <h2>Watch Our Story</h2>
              <h4>This video showcases products from ToyStore. We hope you can find and purchase your favorite items!</h4>
              <a href="#" className="play-button w-inline-block w-lightbox"><img src="https://cdn.prod.website-files.com/5badda2935e11303a89a461e/5bae5eec4b504f1c4031af32_play-icon-white.svg" alt=""/></a>
            </div>
          </div>

        </div>

      </div>
      <Footer></Footer>
    </div>
  );
}